...
function ExampleWorkflowContent({ text }: { text: string }) {
  ...
  // Section markers are matched against the raw README (DB content), so the
  // sentinels stay literal English; only the rendered badge labels below are
  // translated via t().
  for (const line of lines) {
    ...
    <div className="space-y-4">
      ...
      {inputText && (
        <div className="rounded-lg border border-blue-100 bg-blue-50/50 p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded">{t("workflowInput")}</span>
          </div>
      ...
      {steps.length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-bold text-purple-600 bg-purple-100 px-2 py-0.5 rounded">{t("workflowAgent")}</span>
          </div>
      ...
      {outputText && (
        <div className="rounded-lg border border-green-100 bg-green-50/50 p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded">{t("workflowOutput")}</span>
          </div>
  ...
  if (!skill) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        <p className="text-lg text-gray-500">{t("skillNotFound")}</p>
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="p-6 sm:p-8 border-b border-gray-100">
          <div className="flex items-start justify-between">
            <div>
              ...
              {skill.author && (
                <p className="text-sm text-gray-500 mt-1">{t("by", { author: skill.author })}</p>
              )}
            ...
            <div className="flex items-center gap-3 flex-wrap justify-end">
              {skill.downloads > 0 && (
                <div className="flex items-center gap-1.5 text-sm text-gray-400" title={t("downloads")}>
                  <Download className="h-4 w-4" />
                  {formatNumber(skill.downloads)}
                </div>
              )}
              {skill.stars > 0 && (
                <div className="flex items-center gap-1.5 text-sm text-gray-400" title={t("stars")}>
                  <Star className="h-4 w-4" />
                  {formatNumber(skill.stars)}
                </div>
              )}
              <div className="flex items-center gap-1.5 text-sm text-gray-400" title={t("views")}>
        ...
        <div className="p-6 sm:p-8 border-b border-gray-100 space-y-4">
          ...
          {skill.installCmd && (
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t("clawHubCli")}</label>
        ...
        <div className="p-6 sm:p-8 space-y-3">
          <h2 className="text-sm font-semibold text-gray-900 mb-3">{t("links")}</h2>
