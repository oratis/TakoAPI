...
const PROTOCOL_OPTIONS = [
  { key: "A2A", label: "A2A" },
  { key: "OPENAI_COMPAT", label: "OpenAI-compatible" },
  { key: "MCP", label: "MCP" },
];
const PRICING_OPTIONS = [
  { key: "FREE", label: "Free" },
  { key: "PER_CALL", label: "Per call" },
  { key: "PER_TASK", label: "Per task" },
  { key: "PER_TOKEN", label: "Per 1k tokens" },
];
...
export default function SubmitAgentPage() {
  ...
  if (!session) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl font-bold mb-2">Publish an Agent</h1>
        <p className="text-gray-500 mb-6">Please sign in to publish an agent.</p>
        <a
          href="/auth/signin"
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-600 mb-1">
          <span className="font-medium">{done.name}</span> is now pending review.
        </p>
        <p className="text-sm text-gray-400 mb-6">
          We&apos;ll verify the AgentCard and publish it to the marketplace once approved.
        </p>
        <a href="/agents" className="text-purple-600 text-sm font-medium hover:underline">
          ← Back to marketplace
        </a>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <h1 className="text-2xl font-bold mb-1">Publish an Agent</h1>
      <p className="text-sm text-gray-500 mb-8">
        List an invokable agent on the marketplace. Submit its A2A AgentCard URL and we&apos;ll parse
        the rest, or fill it in manually.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {mode === "url" ? (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">AgentCard URL</label>
            <input
              type="url"
              required
              value={form.cardUrl}
            ...
            <p className="text-xs text-gray-400 mt-1">
              We fetch and validate the A2A AgentCard (name, endpoint, capabilities, skills).
            </p>
          </div>
        ) : (
          <>
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder="https://your-agent.com/a2a"
              />
            </div>
            <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Homepage <span className="text-gray-400 font-normal">- optional</span>
          </label>
          <input
            type="url"
            value={form.homepage}
        ...
        >
          {submitting ? "Submitting…" : "Publish Agent"}
        </button>
      </form>
    </div>
  );
